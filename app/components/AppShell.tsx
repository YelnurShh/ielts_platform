  ...
  return <div className="min-h-screen flex flex-col">
    <header className="sticky top-0 z-50 border-b border-[#e6e8ee] bg-white/90 backdrop-blur-xl">
     <div className="container-wide h-[64px] flex items-center justify-between gap-3">
         ...
         {user ? (
           <>
             <div className="hidden md:flex items-center gap-2 px-3 py-2 rounded-xl border border-[#e6e8ee] bg-white shrink-0">
               ...
               <button onClick={handleLogout} title={t.ui.logout} className="hidden md:inline-flex p-2 rounded-xl border border-[#e6e8ee] bg-white hover:bg-[#f8f8fa]"><LogOut size={18}/></button>
            </>
          ) : (
      ...
      {mobile&&<div className="lg:hidden border-t border-[#e6e8ee] bg-white"><div className="container-wide py-3 grid gap-1">{navKeys.map(([href,key,Icon])=><Link onClick={()=>setMobile(false)} key={href} href={href} className={`nav-link ${path===href?"active":""}`}><span className="flex items-center gap-2"><Icon size={16}/>{t.nav[key as keyof typeof t.nav]}</span></Link>)}</div></div>}
    </header>
    <main className="flex-1">{children}</main>
    <footer className="border-t border-[#e6e8ee] bg-white mt-16">
      <div className="container-wide py-9 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <div className="font-extrabold">IELTS Writing Mastery</div>
          <p className="text-sm text-[#667085] mt-1">{t.home.learningLoop}</p>
        </div>
        <div className="text-sm text-[#667085]">{new Date().getFullYear()} • {t.footer.rights.replace("{year}", String(new Date().getFullYear()))}</div>
      </div>
    </footer>
  </div>
}
